...
rates_ger_p = [
    prod_ger.loc[prod_ger["NumOfProducts"] == p, "churn_pct"].values[0]
    if p in prod_ger["NumOfProducts"].values
    else 0
    for p in products
]
rates_other_p = [
    prod_other.loc[prod_other["NumOfProducts"] == p, "churn_pct"].values[0]
    if p in prod_other["NumOfProducts"].values
    else 0
    for p in products
]
...
ax.set_title(
    "Germany Product Churn:\n1-Product Gap Is the Key Differentiator",
    fontsize=13,
    fontweight="bold",
    color="#2c3e50",
)
...
ax.set_title(
    "Active Member Rate by Geography:\nGermany Not Less Engaged Overall",
    fontsize=13,
    fontweight="bold",
    color="#2c3e50",
)
...
plt.tight_layout()
plt.savefig(
    "outputs/finding7_germany_deepdive.png", dpi=150, bbox_inches="tight", facecolor="white"
)
plt.close()
