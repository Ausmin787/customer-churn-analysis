...
geo_hi = (
    df[df["CreditScore"] >= 700].groupby("Geography")["Exited"].agg(["mean", "count"]).reset_index()
)
...
sns.kdeplot(
    ...
    alpha=0.45,
    linewidth=2,
    label=f"Stayed (avg {mean_stayed:.0f})",
    ax=ax,
)
sns.kdeplot(
    churned,
    ...
    alpha=0.45,
    linewidth=2,
    label=f"Churned (avg {mean_churned:.0f})",
    ax=ax,
)
...
bars = ax.bar(
    band_churn["cs_band"].astype(str),
    band_churn["churn_pct"],
    color=bar_colors,
    width=0.6,
    edgecolor="white",
    linewidth=1.5,
...
ax.set_title(
    "Germany Churn at High Credit Score (700+):\nAnomaly Persists",
    fontsize=13,
    fontweight="bold",
    color="#2c3e50",
)
